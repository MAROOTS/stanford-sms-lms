...
export default function Sidebar() {
  ...
  return (
    <aside
      className={`${collapsed ? "w-16" : "w-64"} bg-navy-900 text-white flex flex-col h-screen sticky top-0 transition-all`}
    >
      <div className="flex items-center gap-2 px-5 py-5">
        <div className="w-8 h-8 rounded-lg bg-teal-accent flex items-center justify-center font-bold text-sm">
          🏛
        </div>
        {!collapsed && <span className="font-bold text-lg">StanfordOS</span>}
      </div>
      ...
      <button
        ...
        className="flex items-center gap-2 px-5 py-4 text-sm text-slate-400 hover:text-white border-t border-white/5"
      >
        <ChevronsLeft
          size={16}
