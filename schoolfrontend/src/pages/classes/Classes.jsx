        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-8">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Manage your school's classes, organized by grade level and stream.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
            <button
                ...
                className="w-full sm:w-auto flex items-center justify-center gap-2 border border-slate-200 bg-white text-slate-800 hover:bg-slate-50 shadow-sm text-sm font-semibold px-5 py-2.5 rounded-xl transition-all active:scale-[0.98]"
            >
                  ...
                  {sections.map((s) => (
                      ...
                      >
                        <td className="px-6 py-4 font-semibold text-slate-900 whitespace-nowrap">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-600 font-bold text-xs shrink-0">
                              {s.name?.substring(0, 2).toUpperCase() || 'CL'}
                            </div>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          {s.gradeLevelStage ? (
                              <span className="inline-flex items-center gap-1.5 bg-slate-100 border border-slate-200 text-slate-700 text-xs font-semibold px-2.5 py-1 rounded-lg">
                          <GraduationCap
                              size={14}
        ...
        {promoteOpen && (
            <PromoteClassModal
                ...
                onDone={(data) => {
                  ...
                  toast.success(
                      `${data.moved} student(s) moved from ${data.fromClassName} to ${data.toClassName}`
                  );
