...
export default function FeeCollection() {
    ...
    const loadTermData = useCallback(async () => {
        ...
        try {
            ...
            const [invoicesRes, summaryRes] =
                await Promise.all([
                    axiosClient.get(
                        `/fee-invoices/term/${termId}`,
                        { params }
                    ),
                    axiosClient.get(
                        `/fee-invoices/term/${termId}/summary`,
{ params }
),
]);
...
setSummary(summaryRes.data);
    ...
    setError(
        readApiError(err, {
            ...
            error:
                'Could not load fee data for this term.',
        }).description
    );
} finally {
    setLoading(false);
    ...
    <div>
        <div className="flex items-start justify-between mb-6 flex-wrap gap-4">
            <div>
                ...
                <p className="text-sm text-slate-500 mt-1">
                    Invoices, payments, and collection summary by term.
                </p>
            </div>
        ...
        {summary && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-5">
                    <div className="w-10 h-10 rounded-lg bg-teal-accent/15 flex items-center justify-center mb-4">
                        <CheckCircle2
                            size={18}
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-5">
                    ...
                    <p className="text-2xl font-bold text-slate-900">
                        {formatKES(summary.outstandingBalance)}
                    </p>
                            ...
                            >
                                ...
                                <td className="px-6 py-4">
                                    {inv.balance > 0 ? (
                                        ...
                                        <span className="inline-block bg-teal-accent/15 text-teal-700 text-xs font-medium px-2.5 py-1 rounded-full">
                                                    Paid in full
                                                </span>
                                    )}
                                </td>
                ...
                summary.collectionByMethod.length > 0 ? (
                    <>
                        ...
                        >
                            <PieChart>
                                <Pie
                                    data={summary.collectionByMethod.map(
                                        (item, i) => ({
                                            ...item,
                                            fill:
                                                CHART_COLORS[
