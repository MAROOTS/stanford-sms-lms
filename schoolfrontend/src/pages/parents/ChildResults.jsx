...
export default function ChildResults() {
        ...
        axiosClient
            ...
            .catch((err) => {
                setNotice(
                    readApiError(err, {
                        forbidden:
                            'You can only view results for your linked children.',
                        notFound: 'The selected student could not be found.',
    ...
    // Load exams available to the child's class
    useEffect(() => {
        ...
        axiosClient
            ...
            .catch((err) => {
                setNotice(
                    readApiError(err, {
                        forbidden:
                            'You can only view exams for your linked children.',
    ...
    const loadResult = useCallback(async () => {
        ...
        try {
            const { data } = await axiosClient.get(
                `/results/student/${childId}/exam/${examId}`
            );
            ...
            setNotice(
                readApiError(err, {
                    forbidden:
                        'You can only view results for your linked children.',
                    notFound:
                        'No results have been posted for this exam yet.',
    ...
    const handleDownloadReportCard = async () => {
        ...
        try {
            const response = await axiosClient.get(
                `/report-cards/student/${childId}/exam/${examId}`,
                {
                    responseType: 'blob',
                }
            );
            ...
            // may arrive as a Blob instead of normal JSON.
            if (
                response.data.type &&
            ...
            if (!win) {
                toast.warning(
                    'Pop-up blocked. Please allow pop-ups for this site to view the report card.'
                );
            }
        } catch (err) {
            const parsed = await readApiErrorAsync(err, {
                forbidden:
                    'You can only download report cards for your linked children.',
                notFound:
                    'Marks may not be posted for this exam yet.',
                error: 'Could not generate the report card.',
            });
    ...
    return (
        <div>
            ...
            {!loading && notice && (
                <NoticeCard
                    notice={notice}
                    onRetry={examId ? loadResult : undefined}
                />
            )}
        ...
        {result.subjectResults.map((s) => (
            ...
            >
                ...
                <td className="px-6 py-3">
                                            <span
                                                className={`inline-block text-xs font-medium px-2.5 py-1 rounded-full ${getGradeStyle(
                                                    s.grade
                                                )}`}
                                            >
                                                {s.grade}
...
{!loading && !result && !notice && !examId && (
    <div className="bg-white rounded-xl border border-slate-200 p-10 text-center">
        ...
        <p className="text-sm text-slate-400 max-w-sm mx-auto">
            Select an exam above to see{' '}
            {child?.firstName}'s detailed results,
            including per-subject scores and class
            ranking.
        </p>
    </div>
)}
